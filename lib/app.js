...
export function createApp({ store, env = process.env, serveStatic = false, publicDir } = {}) {
  ...
  async function searchAncient(q) {
    ...
    let list = [];
    // 정확→시작→(뜻풀이 검색: 현대어로 대응 옛말 찾기, 예 '나무'→'나모')→(표제어 포함: 옛형태 직접 입력 대응)
    for (const step of [
      { target: 1, method: "exact", num: 20 },
      { target: 1, method: "start", num: 20 },
      { target: 9, method: "include", num: 30 },
      { target: 1, method: "include", num: 20 },
    ]) {
      if (list.length) break;
      try { list = await uvalSearch({ q, advanced: "y", type3: "ancient", ...step }); } catch { }
    }
    const qEsc = q.trim().replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    const directRe = new RegExp("[‘’'\"]" + qEsc + "[‘’'\"]\\s*의\\s*옛말");   // "‘나무’의 옛말" 처럼 현대어 직접 대응
    ...
    for (const it of list) {
      ...
      const rank = senses.some(s => s.defs.some(d => directRe.test(d))) ? 0 : 1;   // 현대어 대응 옛말 먼저
      out.push({ headword: head, origin, senses, _r: rank });
    }
    out.sort((a, b) => a._r - b._r);
    return out.slice(0, 20).map(({ _r, ...e }) => e);
  }
  ...
  // 한 글자의 '뜻 단위'들(훈 + 뜻풀이)을 잘게 나눠 반환. 예: "다를/나눌 별" → ["다를","나눌"]
  function meaningUnits(ch) {
    const units = [];
    for (const seg of String(HANJA[ch].read || "").split(",")) {
      const toks = seg.trim().split(/\s+/).filter(Boolean);
      const hun = toks.length >= 2 ? toks.slice(0, -1).join(" ") : (toks[0] || "");   // 마지막 토큰=음
      for (const w of hun.split(/[\/·]/)) { const s = w.trim(); if (s) units.push(s); }
    }
    for (const d of (HANJA[ch].defs || [])) for (const w of String(d).split(/[\/·,]/)) { const s = w.trim(); if (s) units.push(s); }
    return units;
  }
  function hanjaBrowse(t, mode) {
    let chars = [];
    for (const ch in HANJA) {
      const { eum } = parseRead(HANJA[ch].read);
      let r;
      if (mode === "hun") {
        const units = meaningUnits(ch);
        if (!units.some(u => u.includes(t))) continue;
        // 뜻이 정확히 t면 0(맨 앞), 아니면 t를 포함하는 가장 짧은 뜻의 글자 수(2자→3자…)
        if (units.some(u => u === t)) r = 0;
        else { r = Infinity; for (const u of units) if (u.includes(t)) r = Math.min(r, u.length); }
      } else {
        if (eum !== t) continue;
        r = 0;
      }
      chars.push({ char: ch, read: HANJA[ch].read, grade: HANJA[ch].grade, r });
    }
    // 1순위: 정확한 뜻/글자수(r)  2순위: 급수(자주 쓰는 순)
    chars.sort((a, b) => (a.r - b.r) || (gradeRank(b.grade) - gradeRank(a.grade)));
    chars = chars.slice(0, 80).map(c => ({ char: c.char, read: c.read }));
    return [{ kind: "browse", mode: mode === "hun" ? "hun" : "eum", query: t, chars }];
  }
  async function searchHanja(q, mode = "eum", scope = "char") {
    const t = q.trim();
    const cjk = [...t].filter(isCJK);
    const hangul = /[\uAC00-\uD7A3]/.test(t);
    // 단어 검색: 한자어 우선(없으면 CJK는 글자 정보로 폴백)
    if (scope === "word") {
      const words = await hanjaWordLookup(t);
      if (words.length) return words;
      if (cjk.length) return (await Promise.all(cjk.map(hanjaEntry))).filter(Boolean);
      return [];
    }
    // 한자(글자) 검색: 한글 한 음절 입력 → 음/뜻으로 한자 찾아보기
  ...
  app.get("/search", async (req, res) => {
    ...
    try {
      ...
      else if (dict === "hanja") results = await searchHanja(query, String(req.query.mode || "eum"), String(req.query.scope || "char"));
